...
def get_X(dat_path, r_mode):
    ...
    if r_mode == "org":
        ...
        radius_data = np.array([radius]).astype(np.float32)
        # one-hot encoding
        ohe_cols = []
        for i in range(0,19):
        ...
        ohe_value = np.array(radius_data).reshape(-1,1)
    ...
    cis_1_data.extend(cis_2_data)
    
    cis_data = np.array([cis_1_data]).astype(np.float32)
    
    return cis_data, radius_data
...
def get_result(dat_path, r_mode, model, output_path):
    # 7. Predic
    cis_data, radius_data = get_X(dat_path, r_mode)
    try:
        inputs = {
            model.get_inputs()[0].name: radius_data,
            model.get_inputs()[1].name: cis_data
        }
